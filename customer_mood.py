...
SPRITE_SCALING_CUSTOMER = 0.2
SPRITE_SCALING_ENEMY = 0.005
SPRITE_SCALING_BULLET = 0.005
...
class Customer(arcade.Sprite):
    def __init__(self, bar_list: arcade.SpriteList) -> None:
        self.image_file_name = f"images\C3.png"

        super().__init__(
            self.image_file_name,
            scale=SPRITE_SCALING_CUSTOMER,
        )
        self.indicator_bar: IndicatorBar = IndicatorBar(
            self, bar_list, (self.center_x, self.center_y)
        )
        self.health: int = CUSTOMER_HEALTH
...
class IndicatorBar:
    ...
    :param Customer owner: The owner of this indicator bar.
...
class MyGame(arcade.Window):
    def __init__(self) -> None:
        ...
        self.customer_sprite = Customer(self.bar_list)
    ...
    def setup(self) -> None:
        ...
        self.customer_sprite.position = self.width // 2, self.height // 4
    ...
    def on_mouse_motion(self, x: float, y: float, dx: float, dy: float) -> None:
        ...
        self.customer_sprite.position = x, y

    def on_update(self, delta_time) -> None:
        """Movement and game logic."""
        # Check if the player is dead. If so, exit the game
        if self.customer_sprite.health <= 0:
            arcade.exit()
        ...
        self.customer_sprite.indicator_bar.position = (
            self.customer_sprite.center_x,
            self.customer_sprite.center_y + INDICATOR_BAR_OFFSET,
        )
        ...
        if self.enemy_timer >= ENEMY_ATTACK_COOLDOWN:
            ...
            diff_x = self.customer_sprite.center_x - self.enemy_sprite.center_x
            diff_y = self.customer_sprite.center_y - self.enemy_sprite.center_y
        ...
        for existing_bullet in self.bullet_list:
            ...
            if arcade.check_for_collision(existing_bullet, self.customer_sprite):
                # Damage the player and remove the bullet
                self.customer_sprite.health -= BULLET_DAMAGE
                existing_bullet.remove_from_sprite_lists()

                # Set the player's indicator bar fullness
                self.customer_sprite.indicator_bar.fullness = (
                        self.customer_sprite.health / CUSTOMER_HEALTH
                )
